...
export default function Partners() {
  ...
  async function fetchPartners() {
    ...
    if (partnerUsers) {
      const enriched = await Promise.all(partnerUsers.map(async p => {
        ...
        const { data: checkins } = await supabase.from('daily_checkins')
          .select('date,is_done').eq('user_id', p.id).gte('date', monthStart)
          .eq('is_done', true)
        const checkinDays = checkins ? [...new Set(checkins.map(c=>c.date))].length : 0
        ...
        const bvRate = Math.round((bv / 1500) * 100)
        const ibvRate = Math.round((ibv / 300) * 100)
    ...
    const { data: pendingData } = await supabase
      .from('users')
      .select('id,referrer_email_pending,created_at')
      .not('referrer_email_pending', 'is', null)
    ...
    if (pendingData) {
      const mine = pendingData.filter(p => p.referrer_email_pending === myEmail)
      setPending(mine)
    }
            ...
            {partners.map(p => (
              <div key={p.id} style={{ background:'#fff',borderRadius:14,padding:14,
                ...
                <div style={{ display:'flex',alignItems:'center',gap:12,marginBottom:12 }}>
                  <div style={{ width:44,height:44,borderRadius:'50%',background:avatarBg(p.name||'?'),
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ display:'flex',alignItems:'center',gap:6 }}>
                      <span style={{ fontSize:15,fontWeight:700,color:'#111827' }}>{p.name||'未命名'}</span>
                ...
                <div style={{ display:'flex',gap:8 }}>
                  {[
                    ...
                    <div key={m.label} style={{ flex:1,background:'#F8FAFC',borderRadius:10,
                      padding:'8px',textAlign:'center' }}>
                      <p style={{ fontSize:11,color:'#9CA3AF',margin:'0 0 4px',fontWeight:600 }}>{m.label}</p>
              ...
              <div style={{ background:'#FFF7ED',borderRadius:14,padding:14,
                ...
                {pending.map(p => (
                  <div key={p.id} style={{ display:'flex',alignItems:'center',
                    ...
                    <div>
                      <p style={{ fontSize:14,color:'#374151',margin:0 }}>?</p>
                      <p style={{ fontSize:12,color:'#9CA3AF',margin:'2px 0 0' }}>
                        填了你的 email 申請加入
                      </p>
                    </div>
